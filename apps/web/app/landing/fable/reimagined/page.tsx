import type { Metadata } from "next";
import Link from "next/link";
import { REIMAGINED } from "../../../_lib/reimagined";

export const metadata: Metadata = {
  title: "Reimagined sites | Heavenly Travel",
  description:
    "Four reimaginings of Heavenly Travel by Claude Fable 5.1: the current WordPress site, and the structures of Airbnb, Agoda and Booking.com.",
};

export default function ReimaginedIndex() {
  return (
    <div className="min-h-screen bg-white font-sans text-neutral-900">
      <main className="mx-auto max-w-4xl px-5 pt-16 pb-32 sm:px-8">
        <div className="flex gap-4 text-sm text-neutral-500">
          <Link href="/landing" className="underline-offset-4 hover:underline">
            Landing variations
          </Link>
          <Link href="/model" className="underline-offset-4 hover:underline">
            Model stats
          </Link>
        </div>
        <h1 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">
          Reimagined sites
        </h1>
        <p className="mt-3 max-w-2xl text-neutral-600">
          Four pages by Claude Fable 5.1. Each borrows the structure of a
          reference site and rebuilds it for coach charter and cars with driver,
          in Overpass with the teal-and-amber palette and the photography from
          heavenlytravel.my.
        </p>

        <ol className="mt-10 space-y-5">
          {REIMAGINED.map((r, i) => (
            <li
              key={r.slug}
              className="rounded-2xl border border-neutral-200 p-5 sm:p-6"
            >
              <div className="flex flex-wrap items-baseline justify-between gap-3">
                <h2 className="text-xl font-semibold">
                  <span className="mr-2 text-neutral-400">{i + 1}.</span>
                  {r.name} · {r.label}
                </h2>
                <div className="flex gap-4 text-sm font-medium">
                  <a
                    href={r.sourceHref}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-neutral-500 underline-offset-4 hover:underline"
                  >
                    Source
                  </a>
                  <Link
                    href={r.href}
                    className="underline-offset-4 hover:underline"
                  >
                    Open page
                  </Link>
                </div>
              </div>
              <p className="mt-1 text-sm text-neutral-500">{r.source}</p>
              <p className="mt-4 text-neutral-700">{r.concept}</p>
              <ul className="mt-4 flex flex-wrap gap-2">
                {r.borrowed.map((b) => (
                  <li
                    key={b}
                    className="rounded-full bg-neutral-100 px-3 py-1 text-xs font-medium text-neutral-700"
                  >
                    {b}
                  </li>
                ))}
              </ul>
            </li>
          ))}
        </ol>
      </main>
    </div>
  );
}
